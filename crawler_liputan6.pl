...
for (my $i=0; $i<=$total_days; $i++){ #variabel $input harus dicasting ke int agar looping berjalan
    ...
    for ($page=1; $page<=5; $page++){
        ...
        $url = "https://liputan6.com/news/indeks/$year/$month/$day?page=$page";
        ...
        foreach my $link (@all_urls){
            my $url = $link->url;
                ...
                if ($url=~"/read/" && !exists $news_links{$link->url}){
                    ...
                    if ($url =~"liputan6.com" && $total_liputan6<=2500){
                      $news_links{$url} = 1;
                      $total_liputan6++;
                    }
                }
...
print "Berhasil mendapatkan ". ($total_liputan6) ."tautan.\nMemulai pengunduhan....";

foreach (sort keys (%news_links)) {
    print OUT "$_\n";
}
print "\nproses selesai!\nURL yang disimpan ". ($total_liputan6) ." \n\n";
close OUT;
